/*
    ...
    Scanline-based video and border rendering
*/
...
// Border helpers
// -----------------------------------------------------------------------------
void ZX48kBoard::logBorderChange( unsigned cycle, unsigned char color )
{
    if( border_event_count_ >= MaxBorderEvents ) {
        return;
    }
    ...
    border_event_cycle_[border_event_count_] = cycle;
    border_event_color_[border_event_count_] = color & 0x07;
    border_event_count_++;
}
...
void ZX48kBoard::beginAudioFrame()
{
    ...
    border_event_count_ = 0;
    logBorderChange( 0, border_color_ );
...
{
    if( (port & 0x01) == 0 ) {
        unsigned char new_border = value & 0x07;
        unsigned char new_beeper = (value >> 4) & 0x01;

        port_fe_latch_ = value;
        mic_level_     = (value >> 3) & 0x01;

        if( new_border != border_color_ ) {
            logBorderChange( cpu_->getCycles(), new_border );
            border_color_ = new_border;
        }
...
// -----------------------------------------------------------------------------
int ZX48k::hardwareLineForOutputLine( int y_out )
{
    // Output layout:
    //   0..23   -> top border   (sampled from the last 24 hardware top-border lines)
    //   24..215 -> paper area   (192 lines, exact 1:1)
    //   216..239-> bottom border(sampled from the first 24 hardware bottom-border lines)
    //
    // Hardware layout (48K, 312 lines total):
    //   0..63   -> top border
    //   64..255 -> paper
    //   256..311-> bottom border

    if( y_out < 24 ) {
        return 40 + y_out;          // last 24 lines of top border
    }
    else if( y_out < 24 + 192 ) {
        return 64 + (y_out - 24);   // paper 1:1
    }
    else {
        return 256 + (y_out - (24 + 192)); // first 24 bottom-border lines
    }
}

unsigned char ZX48k::borderColorForOutputScanline( int y_out ) const
{
    int hw_line = hardwareLineForOutputLine( y_out );
    unsigned cycle = (unsigned)(hw_line * 224);

    unsigned char color = main_board_->border_color_ & 0x07;
    if( main_board_->border_event_count_ > 0 ) {
        color = main_board_->border_event_color_[0];
        for( unsigned i=1; i<main_board_->border_event_count_; i++ ) {
            if( main_board_->border_event_cycle_[i] > cycle ) {
                break;
            }
            color = main_board_->border_event_color_[i];
        }
    }
...
void ZX48k::renderScanline( TBitBlock * bits, int y_out, bool flash_phase )
{
    unsigned char * dst = bits->scanline_data( y_out );
    unsigned char border_ci = (unsigned char)(borderColorForOutputScanline( y_out ) & 0x07);

    // Fill full visible scanline with its border color
    memset( dst, border_ci, ScreenWidth );

    // Paper area?
    if( y_out < BorderTop || y_out >= (BorderTop + SpectrumHeight) ) {
        return;
    }

    int sy = y_out - BorderTop;

    // Render 32 bytes = 256 pixels for this paper scanline
    for( int cell=0; cell<32; cell++ ) {
        int x = cell << 3;

        unsigned short paddr = pixelAddress( x, sy );
        unsigned short aaddr = attrAddress( x, sy );
        ...
        unsigned char ink_ci   = (unsigned char)((bright ? 8 : 0) + ink);
        unsigned char paper_ci = (unsigned char)((bright ? 8 : 0) + paper);

        unsigned char * p = dst + BorderLeft + x;

        p[0] = (pix & 0x80) ? ink_ci : paper_ci;
        p[1] = (pix & 0x40) ? ink_ci : paper_ci;
        p[2] = (pix & 0x20) ? ink_ci : paper_ci;
        p[3] = (pix & 0x10) ? ink_ci : paper_ci;
        p[4] = (pix & 0x08) ? ink_ci : paper_ci;
        p[5] = (pix & 0x04) ? ink_ci : paper_ci;
        p[6] = (pix & 0x02) ? ink_ci : paper_ci;
        p[7] = (pix & 0x01) ? ink_ci : paper_ci;
    }
}
...
TBitmapIndexed * ZX48k::renderVideo()
{
    ...
    for( int y=0; y<ScreenHeight; y++ ) {
        renderScanline( bits, y, flash_phase );
    }
