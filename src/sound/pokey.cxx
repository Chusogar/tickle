/*
    ...
    The bulk of this file is the MAME 0.37b7 POKEY core, included below. Only the
    following adaptations were made for tickle:
      - the MAME header (driver.h) is replaced by the small compatibility shim in
        this section (types, register offsets, and no-op timer/stream stubs);
      - the stream/timer-based init (pokey_sh_start/stop) and the unused host
        read/write/serial/keyboard entry points are omitted; a tickle-friendly
        init (pokey_init_tickle) and a C++ wrapper class (TPokey) are appended at
        the end.
*/
#include <stdio.h>
#include <stdlib.h>
...
// ---------------------------------------------------------------------------
// Minimal MAME compatibility shim
// ---------------------------------------------------------------------------
#ifndef TICKLE_MAME_INTTYPES
#define TICKLE_MAME_INTTYPES
...
/* Minimal POKEY interface descriptor (only the fields the core reads) */
struct POKEYinterface {
    int num;
    int baseclock;
};

/* Timing helpers: the core's timer/stream subsystem is stubbed out because the
   sound CPU that drives POKEY is stepped by the Gauntlet main-board scheduler
   and audio is generated per-frame. */
#define TIME_ONE_SEC        1.0
#define TIME_NEVER          1.0e30
#define TIME_IN_SEC(x)      ((double)(x))
#define TIME_IN_USEC(x)     ((double)(x) * (1.0 / 1000000.0))

static char pokey_dummy_timer;

static inline void * timer_set( double, int, void (*)(int) )  { return &pokey_dummy_timer; }
static inline void * timer_pulse( double, int, void (*)(int) ){ return &pokey_dummy_timer; }
static inline void   timer_reset( void *, double )            {}
static inline void   timer_remove( void * )                   {}
static inline void   timer_enable( void *, int )              {}
/* small non-zero elapsed time keeps the random generator advancing on reads */
static inline double timer_timeelapsed( void * )              { return 0.0001; }

static inline void   stream_update( int, int )                {}
// ---------------------------------------------------------------------------
// Begin MAME POKEY core
// ---------------------------------------------------------------------------
/*
...
// Tickle-friendly init/reset (replaces MAME's stream/timer based pokey_sh_start).
static void pokey_reset_tickle( int chip, int samplerate )
{
    struct POKEYregisters *p = &pokey[chip];

    memset( p, 0, sizeof(struct POKEYregisters) );

    p->samplerate_24_8 = (samplerate) ? (intf.baseclock << 8) / samplerate : 1;
    p->divisor[CHAN1] = 4;
    p->divisor[CHAN2] = 4;
    p->divisor[CHAN3] = 4;
    p->divisor[CHAN4] = 4;
    ...
    p->KBCODE = 0x09;
    p->SKCTL = SK_RESET;
    p->rtimer = &pokey_dummy_timer;
}

static int pokey_tables_ready = 0;

static void pokey_init_tickle( int num, int baseclock, int samplerate )
{
    int chip;

    intf.num = num;
    intf.baseclock = baseclock;

    if( !pokey_tables_ready ) {
        ...
        pokey_tables_ready = 1;
    }

    for( chip = 0; chip < num; chip++ )
        pokey_reset_tickle( chip, samplerate );
}

// Tickle C++ wrapper (single instance backed by the core's chip #0)
TPokey::TPokey( unsigned baseClock, unsigned samplingRate )
{
    pokey_init_tickle( 1, (int)baseClock, (int)samplingRate );
...
void TPokey::reset()
{
    pokey_reset_tickle( 0, (int)samplerate_ );
}

void TPokey::writeReg( unsigned offset, unsigned char value )
{
    pokey_register_w( 0, (int)(offset & 15), (int)value );
}

unsigned char TPokey::readReg( unsigned offset )
{
    return (unsigned char) pokey_register_r( 0, (int)(offset & 15) );
}

void TPokey::update( short * dest, int len )
{
    if( len <= 0 ) return;
    pokey0_update( 0, (INT16 *)dest, len );
}
