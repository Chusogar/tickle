/*
    ...
    The bulk of this file is the MAME 0.37b7 YM2151 core by Jarek Burczynski,
    included verbatim below. Only the following adaptations were made for tickle:
      - the MAME headers (driver.h / ym2151.h / osinline.h) are replaced by the
        small compatibility shim in this section;
      - the USE_MAME_TIMERS build switch is disabled so the core uses its own
        internal Timer A/B counters (no MAME timer subsystem required);
      - a small C++ wrapper class (TYM2151) is appended at the end.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// Forward declarations of the core public API (defined further down)
...
// ---------------------------------------------------------------------------
// Begin MAME YM2151 core
// ---------------------------------------------------------------------------

/*undef this to not use MAME timer system*/
//#define USE_MAME_TIMERS  /* tickle: use internal timers */
...
// ---------------------------------------------------------------------------
// End MAME YM2151 core
// ---------------------------------------------------------------------------

// Tickle C++ wrapper (single instance backed by the core's chip #0)
TYM2151::TYM2151( unsigned clock, unsigned samplingRate )
    : samplingRate_(samplingRate), latch_(0), bufL_(0), bufR_(0), bufLen_(0)
{
    YM2151Init( 1, (int)clock, (int)samplingRate );
    YM2151SetIrqHandler( 0, 0 );
    YM2151SetPortWriteHandler( 0, 0 );
}
...
void TYM2151::writeAddress( unsigned char reg )
{
    latch_ = reg;
}

void TYM2151::writeData( unsigned char value )
{
    YM2151WriteReg( 0, latch_, value );
}

unsigned char TYM2151::readStatus()
{
    return (unsigned char) YM2151ReadStatus( 0 );
}

void TYM2151::ensureBuffers( int len )
{
    if( len > bufLen_ ) {
        delete [] bufL_;
        delete [] bufR_;
...
void TYM2151::update( short * dest, int len )
{
    if( len <= 0 ) return;

    ensureBuffers( len );

    INT16 * buffers[2] = { (INT16 *)bufL_, (INT16 *)bufR_ };
    YM2151UpdateOne( 0, buffers, len );

    for( int i = 0; i < len; i++ ) {
        int mono = ( (int)bufL_[i] + (int)bufR_[i] ) >> 1;
        dest[i] = (short)mono;
    }
}
