/*
    ...
    The bulk of this file is the MAME 0.37b7 TMS5220 core, included verbatim
    below. Only the following adaptations were made for tickle:
      - the MAME headers (driver.h / tms5220.h) are replaced by the small
        compatibility shim in this section;
      - the ROM table include "tms5220r.cpp" becomes "tms5220r.h";
      - a small C++ wrapper class (TTMS5220) is appended at the end.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// ---------------------------------------------------------------------------
// End MAME TMS5220 core
// ---------------------------------------------------------------------------

// Tickle C++ wrapper (single instance backed by the core's static state)
TTMS5220::TTMS5220() : src_(0), srcLen_(0)
{
    tms5220_set_irq( 0 );
    tms5220_reset();
}
...
void TTMS5220::writeData( unsigned char value )
{
    tms5220_data_write( value );
}

unsigned char TTMS5220::readStatus()
{
    return (unsigned char) tms5220_status_read();
}

bool TTMS5220::readyToReceive()
{
    return tms5220_ready_read() != 0;
}

void TTMS5220::ensureSrc( int len )
{
    if( len > srcLen_ ) {
        delete [] src_;
        src_ = new short [ len ];
...
void TTMS5220::update( short * dest, int len, unsigned samplingRate )
{
    if( len <= 0 ) return;

    // TMS5220 runs at 8 kHz; generate the matching number of source samples
    // and resample up to the host rate.
    int n = (int)( (double)len * 8000.0 / (double)samplingRate ) + 1;
    if( n < 1 ) n = 1;

    ensureSrc( n );
    tms5220_process( (INT16 *)src_, (unsigned)n );

    for( int i = 0; i < len; i++ ) {
        int idx = (int)( (long long)i * n / len );
        if( idx >= n ) idx = n - 1;
        dest[i] = src_[idx];
    }
}
